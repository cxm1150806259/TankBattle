...
void EnemyAI::initEnemyAIWithTank(Tank* tank,Bomb* bomb)
{
    mTank=tank;
    mTileMapInfo=tank->getTileMapInfo();
    mEnemyTanks=CCArray::createWithCapacity(4);  //    4?
    mEnemyTanks->retain();
...
//静态方法返回EnmeyAI类指针：
EnemyAI* EnemyAI::createEnemyAIWithTank(Tank* tank,Bomb* bomb)
{
    EnemyAI* enemyTank=new EnemyAI();
    enemyTank->initEnemyAIWithTank(tank,bomb);
    return enemyTank;
}
...
void EnemyAI::move()
{
    ...
    {
        //SimpleAudioEngine::sharedEngine()->playEffect("moving.wav");
        Tank* tank=(Tank*)pObj;
        //如果坦克阻塞，换个方向
        if (tank->getBlock())
...
//    }
    ...
    //坦克和炸弹间的碰撞
    CCObject* pbObj;
    CCARRAY_FOREACH(mEnemyTanks, pbObj)
    {
        
        float bornX = mBomb->getPositionX();
        float bornY = mBomb->getPositionY();
        if (!mBomb->getBlock()) {
            Tank* allNamedTank = (Tank*)pbObj;
            if (IsRectIntersect(allNamedTank->getMovedRect(), mBomb->boundingBox()))
            {
                mEnemyTanks->removeObject(pbObj);
                allNamedTank->remove();
                //爆炸
                mBomb->setVisible(false);
                mBomb->bombEffect(ccp(bornX,bornY));
            ...
            else if (IsRectIntersect(mTank->getMovedRect(), mBomb->boundingBox()))
            {
                mTank->setVisible(false);
                //爆炸
                mBomb->setVisible(false);
