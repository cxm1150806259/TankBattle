...
bool ChoiceScene::init()
{
    ...
    /*********zyf**********/
    CCSprite *SpritebackRed=CCSprite::create("red.png");
    SpritebackRed->setPosition(ccp(szWin.width/2, szWin.height/2));
    SpritebackRed->setScale(2.0);
    this->addChild(SpritebackRed,10);
    /**********************/
    ...
    for (int i = 0; i < 3; ++i)
	{
		...
		addChild(pPaddle,10);//改过，原来addChild(pPaddle);
    ...
    addChild(label1, 10, evt_text);//改过，原来addChild(label1, 0, evt_text);
    
    /*****zyf*****/
//    CCLabelAtlas* label2 = CCLabelAtlas::create("Round", szImgs[i], 16, 32, '.');
//    CCSize szlable2 = label2->getContentSize();
//	label2->setAnchorPoint(ccp(0.5f, 0.5f));
//	label2->setScaleX(szWin.width / sz.width * fSetting[0]);
//	label2->setScaleY(szWin.height / sz.height * fSetting[1]);
//	label2->setPosition(ccp(szWin.width * fSetting[2]-100, szWin.height * fSetting[3]));
//	label2->setColor(color);
//    addChild(label2, 10, evt_text);//改过，原来addChild(label1, 0, evt_text);
    /*************/
	//addChild(label1, 0);
    /*****zyf*************/
...
void ChoiceScene::touchDownAction(CCObject* sender, unsigned int controlEvent)
{
    ...
    {
        //开始对应关卡的场景，稍后添加
        if(m_nRound==0)m_nRound=1;
        CCScene* scene = CityScene::scene(m_nRound);
